        ...
        if(source instanceof Player)
        {
            ...
            if (FactionLogic.getFactionName(player.getUniqueId()) != null)
            {
                if(optionalChatType.isPresent())
                {
                    ...
                    {
                        if (optionalChatType.get().equals(ChatEnum.Everyone)) EagleFactions.ChatList.remove(player.getUniqueId());
                        else EagleFactions.ChatList.replace(player.getUniqueId(), EagleFactions.ChatList.get(player.getUniqueId()), optionalChatType.get());
                    }
                    else
                    {
                ...
                if(optionalChatType.get() != null) player.sendMessage(Text.of(PluginInfo.PluginPrefix, "Changed chat to ", TextColors.GOLD, optionalChatType.get(), TextColors.RESET, "!"));
                else player.sendMessage(Text.of(PluginInfo.PluginPrefix, "Changed chat to ", TextColors.GOLD, "global", TextColors.RESET, "!"));
            }
            else
            {
