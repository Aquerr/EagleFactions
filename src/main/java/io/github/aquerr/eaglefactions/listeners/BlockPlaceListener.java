...
public class BlockPlaceListener extends AbstractListener
{
    ...
    public void onBlockPlace(ChangeBlockEvent.All event)
    {
        ...
        {
            final ServerPlayer player = user;
            ...
            {
                if(!super.getPlugin().getProtectionManager().canPlace(transaction.finalReplacement().location().get().createSnapshot(), player.user(), true).hasAccess())
                {
