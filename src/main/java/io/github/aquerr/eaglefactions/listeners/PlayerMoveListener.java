...
//    }
    ...
    {
        final ServerLocation lastLocation = ServerLocation.of(player.world(), event.originalPosition());
        final ServerLocation newLocation = ServerLocation.of(player.world(), event.destinationPosition());

        if(lastLocation.chunkPosition().equals(newLocation.chunkPosition()))
            return;

        final ServerWorld world = player.world();
        final Vector3i oldChunk = lastLocation.chunkPosition();
        final Vector3i newChunk = newLocation.chunkPosition();
        ...
        if (!oldChunkFactionName.equals(newChunkFactionName))
        {
            ...
            final boolean isCancelled = EventRunner.runFactionAreaEnterEventPre(event, player, optionalNewChunkFaction.orElse(null), optionalOldChunkFaction.orElse(null));
            if(isCancelled)
            {
            ...
            if (!newChunkFactionName.equalsIgnoreCase("SafeZone") && !newChunkFactionName.equalsIgnoreCase("WarZone") && !newChunkFactionName.equalsIgnoreCase("Wilderness"))
            {
                if (!super.getPlugin().getPlayerManager().hasAdminMode(player))
                {
                    if (!getPlugin().getFactionLogic().hasOnlinePlayers(optionalNewChunkFaction.get()) && this.factionsConfig.getBlockEnteringFactions())
                    {
                        ...
                        player.sendMessage(PluginInfo.ERROR_PREFIX.append(Component.text(Messages.YOU_CANT_ENTER_THIS_FACTION + " " + Messages.NONE_OF_THIS_FACTIONS_PLAYERS_ARE_ONLINE, NamedTextColor.RED)));
                        event.setCancelled(true);
                        return;
                    }
                }
            ...
            else if (oldChunkFactionName.equalsIgnoreCase("WarZone") && newChunkFactionName.equalsIgnoreCase("SafeZone"))
            {
                if (!super.getPlugin().getPlayerManager().hasAdminMode(player) && this.factionsConfig.shouldBlockEnteringSafezoneFromWarzone())
                {
                    ...
                    else
                    {
                        ...
                        player.sendMessage(PluginInfo.ERROR_PREFIX.append(Component.text(Messages.YOU_CANT_ENTER_SAFEZONE_WHEN_YOU_ARE_IN_WARZONE, NamedTextColor.RED)));
                        event.setCancelled(true);
                        return;
                    }
                }
            ...
            if (this.chatConfig.shouldShowFactionEnterPhrase())
            {
                ...
                if(optionalNewChunkFaction.isPresent())
                {
                    information = Component.empty()
                            .append(MessageLoader.parseMessage(Messages.YOU_HAVE_ENTERED_FACTION, NamedTextColor.WHITE, ImmutableMap.of(Placeholders.FACTION_NAME, Component.text(newChunkFactionName, NamedTextColor.GOLD))));
...
//                    }
            ...
            EventRunner.runFactionAreaEnterEventPost(event, player, optionalNewChunkFaction.orElse(null), optionalOldChunkFaction.orElse(null));
        }

        //Is there any better way for doing this? :O
        //I am feeling bad that I needed to write something like this...
        Sponge.server().scheduler().submit(Task.builder()
                .plugin(Sponge.pluginManager().fromInstance(this.getPlugin()).get())
