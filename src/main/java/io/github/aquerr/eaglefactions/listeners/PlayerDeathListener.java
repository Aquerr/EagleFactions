...
public class PlayerDeathListener extends AbstractListener
{
    ...
    public void onPlayerDeath(final DestructEntityEvent.Death event, final @Getter("entity") ServerPlayer player)
    {
        CompletableFuture.runAsync(() -> super.getPlugin().getPowerManager().decreasePower(player.uniqueId()))
                .thenRun(() -> player.sendMessage(PluginInfo.PLUGIN_PREFIX.append(MessageLoader.parseMessage(Messages.YOUR_POWER_HAS_BEEN_DECREASED_BY, NamedTextColor.WHITE, ImmutableMap.of(Placeholders.NUMBER, Component.text(this.powerConfig.getPowerDecrement(), NamedTextColor.GOLD))).append(Component.newline())
                        .append(Component.text(Messages.CURRENT_POWER + " " + super.getPlugin().getPowerManager().getPlayerPower(player.uniqueId()) + "/" + super.getPlugin().getPowerManager().getPlayerMaxPower(player.uniqueId()), NamedTextColor.GRAY)))));
        ...
        if (this.protectionConfig.getWarZoneWorldNames().contains(((TextComponent)player.world().properties().displayName().get()).content()) || (optionalChunkFaction.isPresent() && optionalChunkFaction.get().isWarZone()))
        {
