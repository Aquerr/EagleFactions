...
public final class ParticlesUtil
{
	...
	{
		final Optional<ServerWorld> optionalWorld = Sponge.server().worldManager().world(ResourceKey.builder().value(claim.getWorldUUID().toString()).build());
		...
		world.playSound(Sound.sound(SoundTypes.ENTITY_EXPERIENCE_ORB_PICKUP.get(), Sound.Source.MASTER, 5, 10), position);
	}
	...
	public static void spawnRemoveAccessParticles(final Claim claim)
	{
		...
		world.playSound(Sound.sound(SoundTypes.ITEM_FIRECHARGE_USE.get(), Sound.Source.MASTER, 5, -10), position);
	}
	...
	public static void spawnClaimParticles(final Claim claim)
	{
		...
		world.playSound(Sound.sound(SoundTypes.BLOCK_ENDER_CHEST_OPEN.get(), Sound.Source.MASTER, 5, -20), position);
	}
	...
	public static void spawnUnclaimParticles(final Claim claim)
	{
		...
		world.playSound(Sound.sound(SoundTypes.ENTITY_SHULKER_SHOOT.get(), Sound.Source.MASTER, 5, -20), position);
	}
	...
	public static void spawnDestroyClaimParticles(final Claim claim)
	{
		...
		world.playSound(Sound.sound(SoundTypes.ENTITY_BLAZE_SHOOT.get(), Sound.Source.MASTER, 5, -20), position);
	}
	...
	public static class HomeParticles implements EagleFactionsConsumerTask<ScheduledTask>
	{
		...
		public void accept(ScheduledTask task)
		{
			...
			if (!this.lastBlockPosition.equals(this.player.location().blockPosition()) || !this.player.isOnline())
				task.cancel();
		}
	}
}
