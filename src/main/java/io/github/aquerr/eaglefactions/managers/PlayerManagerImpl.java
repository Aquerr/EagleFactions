...
public class PlayerManagerImpl implements PlayerManager
{
    ...
    public PlayerManagerImpl(final Supplier<UserManager> userManager, final StorageManager storageManager, final FactionLogic factionLogic, final FactionsConfig factionsConfig, final PowerConfig powerConfig)
    {
    ...
    {
        return userManager.get().find(playerUUID);
    }

    @Override
    public boolean hasAdminMode(final ServerPlayer player)
    {
        return hasAdminMode(player.user());
    }

    @Override
    public boolean hasAdminMode(User user)
    {
        return user.hasPermission(PluginPermissions.CONSTANT_ADMIN_MODE) || this.adminModePlayers.contains(user.uniqueId());
    }

    @Override
    public boolean activateAdminMode(final ServerPlayer player)
    {
        return this.adminModePlayers.add(player.uniqueId());
    }

    @Override
    public boolean deactivateAdminMode(final ServerPlayer player)
    {
