...
public class InvitationManagerImpl implements InvitationManager
{
    ...
    public boolean sendTruceRequest(ServerPlayer player, Faction sourceFaction, Faction targetFaction)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                player.sendMessage(TextComponent.ofChildren(PluginInfo.PLUGIN_PREFIX, MessageLoader.parseMessage(Messages.YOU_HAVE_INVITED_FACTION_TO_THE_TRUCE, GREEN, ImmutableMap.of(Placeholders.FACTION_NAME, text(targetFaction.getName(), GOLD)))));
    ...
    public boolean sendArmisticeOrWarRequest(ServerPlayer player, Faction sourceFaction, Faction targetFaction)
    {
        ...
        if(hasAdminMode)
        {
            if (sourceFaction.isEnemy(targetFaction))
            {
                ...
                player.sendMessage(TextComponent.ofChildren(PluginInfo.PLUGIN_PREFIX, MessageLoader.parseMessage("You force set armistice between you and " + Placeholders.FACTION_NAME.getPlaceholder(), GREEN, ImmutableMap.of(Placeholders.FACTION_NAME, text(targetFaction.getName(), GOLD)))));
            }
            else
            {
                forceWarBetween(sourceFaction, targetFaction);
                player.sendMessage(TextComponent.ofChildren(PluginInfo.PLUGIN_PREFIX, MessageLoader.parseMessage("You force set war between you and " + Placeholders.FACTION_NAME.getPlaceholder(), GREEN, ImmutableMap.of(Placeholders.FACTION_NAME, text(targetFaction.getName(), GOLD)))));
            }
            return true;
        }
        else
    ...
    public void acceptAllyRequest(AllyRequest allyRequest)
    {
        ...
        optionalSenderFactionLeader.ifPresent(x -> optionalSenderFactionLeader.get().sendMessage(TextComponent.ofChildren(PluginInfo.PLUGIN_PREFIX, MessageLoader.parseMessage(Messages.FACTION_ACCEPTED_YOUR_INVITE_TO_THE_ALLIANCE, GREEN, ImmutableMap.of(Placeholders.FACTION_NAME, text(allyRequest.getInvitedFaction(), GOLD))))));
        senderFaction.getOfficers().forEach(x-> this.playerManager.getPlayer(x)
                .ifPresent(y -> TextComponent.ofChildren(PluginInfo.PLUGIN_PREFIX, MessageLoader.parseMessage(Messages.FACTION_ACCEPTED_YOUR_INVITE_TO_THE_ALLIANCE, GREEN, ImmutableMap.of(Placeholders.FACTION_NAME, text(allyRequest.getInvitedFaction(), GOLD))))));
    ...
    public void acceptTruceRequest(TruceRequest truceRequest)
    {
        ...
        final Optional<ServerPlayer> optionalSenderFactionLeader = this.playerManager.getPlayer(senderFaction.getLeader());
        optionalSenderFactionLeader.ifPresent(x -> optionalSenderFactionLeader.get().sendMessage(TextComponent.ofChildren(PluginInfo.PLUGIN_PREFIX, MessageLoader.parseMessage(Messages.FACTION_ACCEPTED_YOUR_INVITE_TO_THE_TRUCE, GREEN, ImmutableMap.of(Placeholders.FACTION_NAME, text(truceRequest.getInvitedFaction(), GOLD))))));
        senderFaction.getOfficers().forEach(x-> this.playerManager.getPlayer(x)
                .ifPresent(y -> TextComponent.ofChildren(PluginInfo.PLUGIN_PREFIX, MessageLoader.parseMessage(Messages.FACTION_ACCEPTED_YOUR_INVITE_TO_THE_TRUCE, GREEN, ImmutableMap.of(Placeholders.FACTION_NAME, text(truceRequest.getInvitedFaction(), GOLD))))));
    ...
    public void acceptArmisticeRequest(ArmisticeRequest armisticeRequest)
    {
        ...
        senderFactionLeader.ifPresent(x->x.sendMessage(TextComponent.ofChildren(PluginInfo.PLUGIN_PREFIX, MessageLoader.parseMessage(Messages.FACTION_ACCEPTED_YOUR_ARMISTICE_REQUEST, GREEN, ImmutableMap.of(Placeholders.FACTION_NAME, text(armisticeRequest.getInvitedFaction(), GOLD))))));
        senderFaction.getOfficers().forEach(x-> this.playerManager.getPlayer(x)
                .ifPresent(y->y.sendMessage(TextComponent.ofChildren(PluginInfo.PLUGIN_PREFIX, MessageLoader.parseMessage(Messages.FACTION_ACCEPTED_YOUR_ARMISTICE_REQUEST, GREEN, ImmutableMap.of(Placeholders.FACTION_NAME, text(armisticeRequest.getInvitedFaction(), GOLD)))))));
    ...
    private boolean declareWar(Player player, Faction sourceFaction, Faction targetFaction)
    {
        ...
        player.sendMessage(TextComponent.ofChildren(PluginInfo.PLUGIN_PREFIX, MessageLoader.parseMessage(Messages.YOUR_FACTION_IS_NOW_ENEMIES_WITH_FACTION, NamedTextColor.WHITE, ImmutableMap.of(Placeholders.FACTION_NAME, text(targetFaction.getName(), GOLD))).color(RED)));
    ...
    {
        final TextComponent clickHereText = Component.empty()
                ...
                .hoverEvent(text("/f join " + senderFaction.getName(), GOLD)));

        return TextComponent.ofChildren(PluginInfo.PLUGIN_PREFIX,
                MessageLoader.parseMessage(Messages.FACTION_HAS_SENT_YOU_AN_INVITE, GREEN, ImmutableMap.of(Placeholders.FACTION_NAME, text(senderFaction.getName(), GOLD))),
                text(Messages.YOU_HAVE_TWO_MINUTES_TO_ACCEPT_IT),
                Component.newline(),
                clickHereText,
                text(" " + Messages.TO_ACCEPT_INVITATION_OR_TYPE + " ", GREEN),
    ...
    private TextComponent getAllyInviteGetMessage(final Faction senderFaction)
    {
        final TextComponent clickHereText = Component.empty()
                ...
                .hoverEvent(text("/f ally " + senderFaction.getName(), GOLD));

        return TextComponent.ofChildren(PluginInfo.PLUGIN_PREFIX, MessageLoader.parseMessage(Messages.FACTION_HAS_SENT_YOU_AN_INVITE_TO_THE_ALLIANCE, GREEN, ImmutableMap.of(Placeholders.FACTION_NAME, text(senderFaction.getName(), GOLD))),
                newline(),
                text(Messages.YOU_HAVE_TWO_MINUTES_TO_ACCEPT_IT),
                newline(),
                clickHereText,
    ...
    {
        final TextComponent clickHereText = empty()
                .append(text("[", AQUA).append(text(Messages.CLICK_HERE, GOLD).append(text("]", AQUA)))
                .clickEvent(ClickEvent.runCommand("/f truce " + senderFaction.getName()))
                .hoverEvent(text("/f truce " + senderFaction.getName(), GOLD)));

        return TextComponent.ofChildren(PluginInfo.PLUGIN_PREFIX, MessageLoader.parseMessage(Messages.FACTION_HAS_SENT_YOU_AN_INVITE_TO_THE_TRUCE, GREEN, ImmutableMap.of(Placeholders.FACTION_NAME, text(senderFaction.getName(), GOLD))),
                newline(),
                text(Messages.YOU_HAVE_TWO_MINUTES_TO_ACCEPT_IT),
                newline(),
                clickHereText,
    ...
    {
        final TextComponent clickHereText = Component.empty()
                .append(TextComponent.ofChildren(text("[", AQUA), text(Messages.CLICK_HERE, GOLD), text("]", AQUA)))
                .clickEvent(ClickEvent.runCommand("/f enemy " + senderFaction.getName()))
                .hoverEvent(text("/f enemy " + senderFaction.getName(), GOLD));

        return TextComponent.ofChildren(PluginInfo.PLUGIN_PREFIX, MessageLoader.parseMessage(Messages.FACTION_HAS_SENT_YOU_AN_ARMISTICE_REQUEST, GREEN, ImmutableMap.of(Placeholders.FACTION_NAME, text(senderFaction.getName(), GOLD))),
                Component.newline(),
                text(Messages.YOU_HAVE_TWO_MINUTES_TO_ACCEPT_IT),
                Component.newline(),
                clickHereText,
                text(" " + Messages.TO_ACCEPT_IT_OR_TYPE + " ", GREEN),
