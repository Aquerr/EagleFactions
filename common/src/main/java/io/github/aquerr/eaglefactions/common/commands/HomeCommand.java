...
{
    public HomeCommand(EagleFactionsPlugin plugin)
    {
        super(plugin);
    }
        ...
        if(source instanceof Player)
        {
            ...
            if(optionalPlayerFaction.isPresent())
            {
                ...
                if(playerFaction.getHome() != null)
                {
                    ...
                    else if (getPlugin().getConfiguration().getConfigFields().shouldBlockHomeAfterDeathInOwnFaction() && EagleFactionsPlugin.BLOCKED_HOME.containsKey(player.getUniqueId()))
                    {
